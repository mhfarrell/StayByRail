  ...
  return (
    <div className="hotel-card">
      ...
      <div className="hotel-info">
              ...
              {showPerNight ? (
                <div className="price-per-night">
                  ...
                  <span className="price-label">
                    per night on {cheapestSite || "booking site"}
                  </span>
                </div>
              ) : (
                <div className="price-total">
                  ...
                  <span className="price-label">
                    {hotel.num_nights}n on {cheapestSite || "booking site"}
                  </span>
                </div>
              )}
              <span className="book-now-badge">
                Book on {cheapestSite || "site"}
              </span>
            </a>
          ) : (
            <div>
        ...
        {/* Wishlist items with traffic lights */}
        {score && score.total > 0 && (
          <div className="wishlist-results">
            {score.items.map((item) => (
              <span
                key={item.name}
                className={`tag wl-${item.status}`}
              >
                {item.status === "yes"
                  ? "✓"
                  : item.status === "no"
            ...
            hotel.amenities
              .filter(
                (a) =>
                  !wishlist ||
                  !wishlist.some(
                    (w) => a.toLowerCase().includes(w.toLowerCase())
                  )
              )
              .slice(0, 5)
              .map((a) => (
                <span key={a} className="tag amenity-tag">
                  {a}
                </span>
              ))}
        </div>
